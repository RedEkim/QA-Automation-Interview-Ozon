...
# 📚 Ресурсы для изучения PostgreSQL & SQL (Недели 7–11)

## 📌 База SQL, Соединения и Подзапросы
* **[Интерактивный учебник] Интерактивный тренажер по SQL на Stepik (Галина Озерова)**  
  Бесплатный и один из лучших курсов для старта. Теория подается короткими блоками и сразу закрепляется решением задач прямо в браузере.
* **[Документация] Официальная документация PostgreSQL (на русском от Postgres Professional)**  
  Разделы *«Язык SQL»* и *«Управление данными»*. Первоисточник, который всегда должен быть под рукой.
* **[Статья] Visual Explanation of SQL Joins**  
  Статья *A Visual Explanation of SQL Joins* (и ее переводы на Хабре) с диаграммами Эйлера поможет визуализировать разницу между `INNER`, `LEFT`, `RIGHT`, `FULL` и `CROSS JOIN`.

---

## ⚙️ Индексы, Транзакции, ACID и Изоляция
* **[Книга / Бесплатный курс] «PostgreSQL изнутри» (Егор Рогов / Postgres Professional)**  
  Главный материал по теме индексов, ACID и уровней изоляции. Книга официально доступна бесплатно в PDF на сайте PostgresPro. Особенно полезны главы про MVCC, индексы (B-Tree, Hash) и устройство транзакций.
* **[Гайд] Use The Index, Luke! (Markus Winand)**  
  Руководство по индексам в БД. Объясняет структуру B-Tree, причины ускорения `SELECT`, накладные расходы при `INSERT/UPDATE/DELETE` и работу `EXPLAIN ANALYZE`.
* **[Видео] YouTube-канал Postgres Professional**  
  Лекции курсов PostgresPro и доклады с конференций PGDay/PGConf. Recommended темы для поиска: *«MVCC в PostgreSQL»*, *«Уровни изоляции транзакций»*, *«Аномалии чтения»*.

---

## 🛠️ Практика и Задачи
* **SQL Academy** — интерактивный тренажер с задачами от простых к сложным и справочником.
* **SQL-EX.ru** — классическая платформа для практики с реальными базами данных (отлично тренирует подзапросы, группировки и `HAVING`).
* **LeetCode (Database / SQL)** — раздел задач уровня *Easy* и *Medium* для закрепления навыков.
...
2 hour
- [Yandex practicum]
  - Как работать с ER диаграммой и описанием базы данных
